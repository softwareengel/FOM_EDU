class WesternDigitalRuleChecker(object):
    ...
    def check_rule_1(self, datapoints):
        ...
        if (len(datapoints) >= 1):
            number_of_points_outside_3_deviations = 0
            item = datapoints[-1]
            if (self.estimated_value + 3 * self.standard_deviation < item):
                number_of_points_outside_3_deviations = number_of_points_outside_3_deviations +1
            if (self.estimated_value - 3 * self.standard_deviation > item):
                number_of_points_outside_3_deviations = number_of_points_outside_3_deviations +1
            if number_of_points_outside_3_deviations >0:
                print("Rule 1 failed")
                return False
        return True
...
if __name__=='__main__':
    wd = WesternDigitalRuleChecker(5, 1)
    
    liste = [1,2,3,4.5,6,7,8.8,88,888,88,88,88,88]
    erg = wd.check_rule_4(liste)
    print (erg)